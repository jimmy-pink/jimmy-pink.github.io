...
### 生物学中的神经元
 ![[Pasted image 20250429183131.png]]
...
![[Pasted image 20250429185705.png]]
...
>  非线性(Non-Linearity)：激活函数引入了非线性，这是模型学习复杂数据模式的关键。
...
## 神经网络 (Neural Network)
![[Pasted image 20250429191726.png]]
...
![[Pasted image 20250429201522.png]]
...
- 工作流程：
	...
	3. 反向传播计算梯度
		...
		- Loss函数计算输出层梯度：
			...
			- $\hat{y}$ 对 W' 求导：$\frac{\partial \hat{y}}{\partial W{\prime}} = a^{(1)}$, $a^{(1)}$是上一层的输出
			- ==根据链式法则， Loss函数对 W‘ 求导==：$\frac{\partial \text{Loss}}{\partial W{\prime}} = (\hat{y} - y) \cdot a^{(1)}$
...
#### 学习率 (Learning Rate)
![[Pasted image 20250430162017.png]]
- 决定每次参数更新的步长。（梯度下降的步长）
- 它是最重要的**超参数**之一
- 挑战：
