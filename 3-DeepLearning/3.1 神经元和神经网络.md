...
1. 线性函数再多再复杂的组合，最终都可以简化为线性函数，y=wx+b；导致失去复杂度。
2. 在线性回归之后加入**激活函数**，才为复合函数（Composite Function）**注入了灵魂**。
...
*比如 y1=wx+b, y2=w1y1+b1, 而y1中的x可能是multi-input, 甚至是多项目式，w又有可能是矩阵也就是y1是多个结果，如此层层套娃 由简入繁 最终构建出连科学家的大脑也无法想象的复杂表达式 然后通过训练模型预测这个庞大繁杂的表达式中的权重和偏执 从而产生出超人智慧的模型和预测能力*
...
![Pasted image 20260205220019.png](../images/Pasted%20image%2020260205220019.png)
例如，我们用线性回归来拟合数据时， 一定会出现噪点和异常。

正是因为<font style="background-color:yellow; color:black">**注入了激活函数，才让模型具备了“无限贴合”现实复杂性的能力。**</font>
...
1. 非线性 (Non-linearity)：打破“线性套娃”的死循环
	- **痛点**：线性组合的线性组合，依然是线性。
	- **必要性**：现实世界的问题（如人脸识别、语言理解）绝不是简单的线性叠加。激活函数给公式注入了“弯曲”的能力。
	- **解决的痛点**：它让模型能够拟合**非线性边界**。有了它，模型才能理解“如果...那么...”这种逻辑跳跃。
2. 可微性 (Differentiability)：给梯度下降提供“指南针”
	- **痛点**：如果你用一个简单的“硬阈值”函数（比如：小于 0 输出 0，大于 0 输出 1），它的导数在跳变点是无穷大，在其他地方是 0。
	- **必要性**：PyTorch 的核心是**自动求导**。如果函数不可微，权重 $W$ 就不知道该往哪个方向修改。
	- **激活函数的特征**：像 **Sigmoid** 或 **Tanh** 这种函数，它们是平滑的曲线，在每一个点都有明确的斜率（梯度）。
	- **解决的痛点**：解决了**模型无法训练**的问题。它让“误差”能够像流水一样，顺着平滑的斜率反向流动到每一层。

当有了 无限拟合现实的“神经元”细胞(线性函数+激活函数)，n个细胞再通过复杂组合排列，形成组织器官，更复杂的组织器官再通过复杂组合，最后得到一个“硅基生命体”，一旦拥有了恰当的足够的输入，经过复杂充分的训练，这个生命体就能够掌握 无限接近物理世界的 真实规律 -- 道（仅指**统计学上的相关性**，而非物理学上的**因果性**，因此目前还不是真正的道）。于是超越人类智慧的硅基生命体就这样诞生了，而这过程的本质 就是算力换真理的过程。
...
![relu-activation.png](../images/relu-activation.png)
...
### 3. **Tanh函数**
![tanh-activation.png](../images/tanh-activation.png)
...
### 4. **Softmax函数**
![softmax-fun.png](../images/softmax-fun.png)
...
![Pasted image 20260216084931.png](../images/Pasted%20image%2020260216084931.png)
- 关键知识点（复习重点）
