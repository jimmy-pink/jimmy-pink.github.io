...
<<<<<<<< HEAD:2-MachineLearning/2.5 回归与分类.md
...
>>>>>>>> origin/obsidian:2-MachineLearning/2.5 监督学习常用算法.md
...
- 定义：能够根据输入数据的特征，将其正确地归入预定义的、离散的</font>类别 (Class) <font style="background-color:yellow; color:black">或</font>标签 (Label) ==中
>>>>>>>> origin/obsidian:2-MachineLearning/2.5 监督学习常用算法.md
