...
![[Pasted image 20250428223720.png]]
...
#### 回归 Regression
[[2.5 监督学习常用算法#回归 (Regression|🔗 回归 (Regression)]]
...
-  [[2.5 监督学习常用算法#简单线性回归 (Simple Linear Regression|简单线性回归 (Simple Linear Regression)]]： 简单数值预测
- [[2.5 监督学习常用算法#普通最小二乘法回归（OLS）|普通最小二乘法回归（OLS）]]
- [[2.5 监督学习常用算法#非线性回归 Nonlinear Regression|非线性回归 Nonlinear Regression]]
...
#### 分类问题 Classification
[[2.5 监督学习常用算法#分类问题 (Classification)|🔗 分类问题 (Classification)]]

 根据输入数据预测**离散类别**

<font style="background-color:tomato; color:white; padding:4px; text-shadow: 1px 1px 2px rgba(0,0,0,0.2);font-weight: bold;">传统机器学习算法：</font>
	
- [[2.5 监督学习常用算法#逻辑回归 (Logistic Regression)|🥏 逻辑回归 Logistic Regression]]   **二分类问题**， 如垃圾邮件标注，是猫还是狗
- [[2.5 监督学习常用算法#🥏 决策树 Decision Tree|🥏 决策树 Decision Tree]]
- [[2.5 监督学习常用算法#🥏 回归树 Regression Tree|🥏 回归树 Regression Tree]]
- [[2.5 监督学习常用算法#🥏 支持向量机 SVM (Support Vector Machine)|🥏 支持向量机 SVM (Support Vector Machine) ]]
- [[2.5 监督学习常用算法#🥏 K-最近邻居 KNN (K-Nearest Neighbors)|🥏 K-最近邻居 KNN (K-Nearest Neighbors)]]
- [[2.5 监督学习常用算法#🥏 随机森林 (Random Forest)|🥏 随机森林 Random Forest]]
- [[2.5 监督学习常用算法#🥏 极端梯度提升 (XGBoost)|🥏 极端梯度提升 XGBoost]]
...
+ 发现数据的潜在结构
+ 数据降维、聚类或生成数据
![[Pasted image 20250418140616.png]]

有监督学习的典型应用：

#### 聚类 Clustering
[[2.6 无监督学习常用算法#聚类 Clustering|聚类 Clustering]]
...
+ [[2.6 无监督学习常用算法#🪼 K-Means|🪼 K-Means]] 
	+ 将数据集划分为 K 个簇，每个簇中心是所有点的平均值, 通过迭代更新所有簇中心，直到收敛为止
    - 应用：客户细分， 图像压缩，目标检测
+ [[2.6 无监督学习常用算法# 🪼 凝聚式层次聚类 (Agglomerative Hierarchical Clustering)|🪼 凝聚式层次聚类 (Agglomerative Hierarchical Clustering)]]
	+ 通过构建树型层次结构，将数据逐层划分
		+ Divisive 自上而下 分裂
		+ Agglomerative 自下而上 合并
    - 应用：物种分类，文本聚类
- [[2.6 无监督学习常用算法#🪼 密度聚类算法 (DBScan)|🪼 密度聚类算法 (DBScan)]](Density-Based Spatial Clustering of Applications with Noise) 
    - 寻找高密区域，将数据点划分为簇
...
+ [[2.6 无监督学习常用算法#🪼 PCA (Principal Component Analysis)|🪼 PCA (Principal Component Analysis)]]：线性降维，保留最大方差（如数据可视化）。 
+ [[2.6 无监督学习常用算法#🪼 t-SNE (t-Distributed Stochastic Neighbor Embedding)|🪼 t-SNE (t-Distributed Stochastic Neighbor Embedding)]]：非线性降维，保留局部结构（如高维数据可视化）。 
+ [[2.6 无监督学习常用算法#🪼 UMAP (Uniform Manifold Approximation and Projection)|🪼 UMAP (Uniform Manifold Approximation and Projection)]]
