...
<font style="background:#F6E1AC;color:#664900">🌰 快速上手 </font>[ ✍️ 多元线性回归by TF](https://github.com/jimmy-pink/colab-playground/blob/main/1traditional-ml/IBM-1-2-2-MultipleLinearRegression.ipynb)
...
[GITHUB-线性回归Torch代码实例](https://github.com/jimmy-pink/colab-playground/blob/main/3pytorch/PyTorch-%E5%85%A5%E9%97%A8-Regression.ipynb)
...
<font style="background-color:yellow; color:black">思考： 用 对数-似然 来 替代 普通似然（概率相乘），改变了运算符，还能得到精确概率吗？</font>
1. 训练阶段：找最优参数 $\theta^*$
	- 这可以理解为伯努利分布(0-1分布)，找到和没找到。因此，无论用什么运算符，只要正负方向一致，得到结果一样。
2. 推理阶段：还原精确P
	- 我们通过指数运算（$e^{\text{log-value}}$）把结果还原成 0 到 1 之间的精确概率。

<font style="background-color:yellow; color:black">为什么必须用对数而不是其他“不改变方向”的运算？</font>
